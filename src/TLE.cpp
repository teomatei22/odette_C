...
void setValsToRec(TLE *tle, ElsetRec *rec)
{
    double xpdotp = 1440.0 / (2.0 * M_PI);  // 229.1831180523293

    rec->elnum = tle->elnum;
    rec->revnum = tle->revnum;
    rec->satnum = tle->objectNum;
    rec->bstar = tle->bstar;
    rec->inclo = orbmath::deg2rad(tle->incDeg);
    rec->nodeo = orbmath::deg2rad(tle->raanDeg);
    rec->argpo = orbmath::deg2rad(tle->argpDeg);
    rec->mo = orbmath::deg2rad(tle->maDeg);
    rec->ecco = tle->ecc;
